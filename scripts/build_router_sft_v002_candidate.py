...
def build_rows() -> list[dict[str, Any]]:
    ...
    add(rows, a, "contact_friction", "Two specialists disagree whether a frictional interface model is conservative enough for a high-consequence report.", "friction_expert_dispute", "contact analysis", "high", "managed_fusion", ["human_review"], ["assumption_check", "overclaim_check", "approval_check"])
    ...
    add(rows, a, "safety_claim", "Review a safety-critical numerical claim in a synthetic paper for publication readiness.", "safety_claim_review", "engineering publication", "critical", "external_expert", ["human_review"], ["safety_approval_check", "overclaim_check", "assumption_check"])
    ...
    add(rows, a, "local_vs_current", "Determine whether a newly announced solver feature exists when local documentation may be outdated.", "current_solver_feature", "solver documentation", "medium", "web_or_rag_required", ["vendor_docs"], ["citation_check", "terminology_check"])
